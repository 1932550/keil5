...
void CountSensor_Init(void)
{
	...
	/*AFIO选择中断引脚*/
	GPIO_EXTILineConfig(GPIO_PortSourceGPIOA, GPIO_PinSource6);//将外部中断的14号线映射到GPIOB，即选择PB14为外部中断引脚
...
{  
	...
	if(CountSensor_Count>15){
	    OLED_Clear(); 
		OLED_ShowString(1, 1, "Pause");Delay_s(2);
		CountSensor_Count=0;
		...
		Delay_s(1);
	}else{  CountSensor_Cou=4; CountSensor_Count=0;
	}
